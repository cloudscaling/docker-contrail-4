...
read -r -d '' contrail_devicemgr_config << EOM
[DEFAULTS]
log_file=${CONFIG_DEVICE_MGR_log_file:-/var/log/contrail/contrail-device-manager.log}
log_level=${CONFIG_DEVICE_MGR_log_level:-SYS_NOTICE}
...
#get_kv
echo "$contrail_devicemgr_config" > /etc/contrail/contrail-device-manager.conf
